...
class UR5_RG2:
    ...
    def __init__(self):
        ...
        # 读取Base和Joint的句柄
        jointHandle = np.zeros((jointNum, 1), dtype=np.int64)
        ...
        for i in range(jointNum):
            _, jpos = vrep.simxGetJointPosition(clientID, int(jointHandle[i]), vrep.simx_opmode_blocking)
            jointConfig[i] = jpos
    ...
    def rotateAllAngle(self, joint_angle):
        ...
        for i in range(jointNum):
            vrep.simxSetJointTargetPosition(clientID, int(jointHandle[i]), joint_angle[i] / RAD2DEG,
    ...
    def rotateCertainAnglePositive(self, num, angle):
        ...
        vrep.simxSetJointTargetPosition(clientID, int(jointHandle[num]), (jointConfig[num] + angle) / RAD2DEG,
                                        vrep.simx_opmode_oneshot)
    ...
    # convert array from v-rep to image
...
def main():
    ...
    pygame.display.set_caption("V-rep capture picture")
    # 循环事件，按住一个键可以持续移动
    ...
    while True:
        ...
        # ig = pygame.image.load("imgTempDep\\frame.png")
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYUP:
                ...
                # save Images
                    ...
                    # cv2.imwrite("saveImg\\depthImg\\" + ran_str + "_depth.png", depthImg)
                    cv2.imwrite("saveImg\\rgbImg\\rgb.jpg", rgbImg)
                    cv2.imwrite("saveImg\\depthImg\\depth.png", depthImg)
                    print("save image")
                # reset angle
                elif event.key == pygame.K_l:
                    robot.rotateAllAngle([0, 0, 0, 0, 0, 0])
                    # bangle = float(eval(input("please input velocity: ")))
                else:
